...
struct EditReminderView: View {
    ...
    @State var selectedFrequency: Int
    ...
    let editReminderUid: String
    @Binding var showReminderDetail: Bool

    
    var body: some View {
        VStack(spacing: 0) {
            TopBackBarView(titleText: Text("Edit Reminder"))
            ...
            ScrollView {
                VStack(spacing: 0) {
                    ...
                    Button {
                        ...
                        deleteReminder(reminderUid: self.editReminderUid)
                        self.showReminderDetail = false
                    } label: {
                        MainButtonView(buttonText: Text("Save Changes"))
                    }
                    .padding(.top, 40)
                    ...
                    Button {
                        ...
                        Text("Delete this reminder")
                            .font(Font.custom("Poppins-Regular", size: 14))
                            .foregroundColor(Color(#colorLiteral(red: 0.86, green: 0.08, blue: 0.08, alpha: 1)))
                    }
                    .padding(.top, 16)
    ...
    private func deleteReminder(reminderUid: String) {
        var willDeleteReminderUids: [String] = []
        for reminder in reminders {
            if reminderUid == reminder.reminderUid {
                willDeleteReminderUids.append(reminder.notificationUid!)
        ...
        notificationManager.deleteLocalNotifications(
            identifiers: willDeleteReminderUids
        )
    }
}
