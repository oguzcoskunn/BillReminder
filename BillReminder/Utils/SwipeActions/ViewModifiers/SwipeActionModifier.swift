...
public extension View {
    ...
    @ViewBuilder
    func addSwipeAction<V1: View, V2: View>(menu: MenuType = .swiped, isSwiped: Binding<Bool> = .constant(false), @ViewBuilder _ content: @escaping () -> TupleView<(Leading<V1>, Trailing<V2>)>) -> some View {
        self.modifier(SwipeAction.init(menu: menu, isSwiped: isSwiped, content))
    }
    
    @ViewBuilder
    func addSwipeAction<V1: View>(menu: MenuType = .swiped, edge: HorizontalAlignment, isSwiped: Binding<Bool> = .constant(false), @ViewBuilder _ content: @escaping () -> V1) -> some View {
        switch edge {
        case .leading:
    ...
    @ViewBuilder
    func addFullSwipeAction<V1: View, V2: View>(menu: MenuType = .swiped,
